...
SELECT TO_CHAR(emp.no, '00000') as no, emp.id as id, emp.name as name, TO_CHAR(emp.birth, 'yyyy-mm-dd') as birth, case emp.gender when '0' then '남자' when '1' then '여자' end as gender, dept.d_code, emp.d_fg FROM TBL_EMP_202304 emp, TBL_DEPT_202304 dept WHERE emp.dept = dept.d_code ORDER BY no