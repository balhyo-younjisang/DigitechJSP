...
select avg(soyo) from (SELECT cust_id, min(ymd), max(ymd), to_date(max(ymd)) - to_date(min(ymd)) as soyo from (
	select cust_id, to_char(sale_date, 'yyyy-mm-dd') as ymd rank() over(partition by cust_id order by sale_date)
 as rank from sales s) where rank <= 2 and cust_id IN (select cust_id from sales s group by cust_id having count(*) >= 2) group by cust_id )
